//! The texture maps, as the viewer gets hold of them.
//!
//! On the desktop a map is read off disk the moment it is wanted. On the web it has to
...
impl Default for Maps {
    ...
    fn default() -> Self {
        Self {
            dir: std::path::Path::new(TEXTURE_DIR),
        }
    }
}
...
mod web {
    ...
    /// The maps fetched so far, from the texture folder beside the page.
    #[derive(Default)]
    pub struct Maps {
    ...
    impl Maps {
        ...
        pub fn ready(&self, files: &[String]) -> bool {
            ...
            for file in wanted {
                ...
                let request = ehttp::Request::get(format!("{TEXTURE_DIR}/{file}"));
